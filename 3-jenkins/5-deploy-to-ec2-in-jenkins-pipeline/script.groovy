...
def deploy() {
    ...
    def cmnds = "./script-cmnds.sh ${IMAGE_TAG}"
    def perm = " chmod +x script-cmnds.sh"
    sshagent(['ec2-ssh']) {
        sh "scp -o StrictHostKeyChecking=no script-cmnds.sh tma@3.108.238.58:/home/tma/"
        sh "scp -o StrictHostKeyChecking=no docker-compose.yaml tma@3.108.238.58:/home/tma/"
        sh "ssh -o StrictHostKeyChecking=no tma@3.108.238.58 ${perm}"
        sh "ssh -o StrictHostKeyChecking=no tma@3.108.238.58 ${cmnds}"
    }
