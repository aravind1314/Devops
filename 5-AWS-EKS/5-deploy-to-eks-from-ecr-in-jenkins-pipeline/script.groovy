...
def pushImage() {
    ...
    withCredentials([usernamePassword(credentialsId:'ecr-cred',usernameVariable:'usr',passwordVariable:'pwd')]){
        sh "echo ${pwd} | docker login -u ${usr} --password-stdin ${REPO_URL}"
...
def pushVersion() {
    ...
    withCredentials([usernamePassword(credentialsId:'gitlab-cred',usernameVariable:'usr',passwordVariable:'pwd')]) {
        sh "git remote set-url origin https://${usr}:${pwd}@gitlab.com/aravind1314/complete-ppln-ecr-eks.git"
        sh "git push -u origin HEAD:main"
    }
    echo "update version is successful"
